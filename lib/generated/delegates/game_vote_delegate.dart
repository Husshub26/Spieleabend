...
class GameVoteDelegate {
  GameVoteDelegate(this._executor);

  final BaseExecutor _executor;

  /// Find a single GameVote by unique field(s)
  Future<GameVote?> findUnique({
    required GameVoteWhereUniqueInput where,
    ...
    return result != null ? GameVote.fromJson(_normalizeForJson(result)) : null;
  }

  /// Find a single GameVote or throw if not found
  Future<GameVote> findUniqueOrThrow({
    required GameVoteWhereUniqueInput where,
  }) async {
    final result = await findUnique(where: where);
    if (result == null) {
      throw Exception('GameVote not found');
    }
    return result;
  }

  /// Find the first GameVote matching criteria
  Future<GameVote?> findFirst({
    GameVoteWhereInput? where,
    GameVoteOrderByInput? orderBy,
    Map<String, dynamic>? include,
  }) async {
    final queryBuilder = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Find multiple GameVotes with optional filters
  Future<List<GameVote>> findMany({
    GameVoteWhereInput? where,
    dynamic orderBy,
    int? take,
    int? skip,
    ...
    if (orderBy is GameVoteOrderByInput)
    ...
    return results
        .map((json) => GameVote.fromJson(_normalizeForJson(json)))
        .toList();
  }

  /// Find multiple GameVotes as raw maps (use with include/computed)
  ...
  /// Find the first GameVote as a raw map (use with include/computed)
  ...
  /// Create a new GameVote
  Future<GameVote> create({required CreateGameVoteInput data}) async {
    ...
    if (result == null) {
      throw Exception('Failed to create GameVote');
    }
    return GameVote.fromJson(_normalizeForJson(result));
  }

  /// Create multiple GameVotes
  Future<int> createMany({required List<CreateGameVoteInput> data}) async {
    final query = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Update a GameVote
  Future<GameVote> update({
    required GameVoteWhereUniqueInput where,
    required UpdateGameVoteInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Update multiple GameVotes
  Future<int> updateMany({
    required GameVoteWhereInput where,
    required UpdateGameVoteInput data,
  }) async {
    final query = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Delete a GameVote
  Future<GameVote> delete({required GameVoteWhereUniqueInput where}) async {
  ...
  /// Delete multiple GameVotes
  Future<int> deleteMany({required GameVoteWhereInput where}) async {
    final query = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Count GameVotes matching criteria
  Future<int> count({GameVoteWhereInput? where}) async {
    final queryBuilder = JsonQueryBuilder()
        .model('GameVote')
  ...
  /// Group GameVotes by fields with aggregations
  ...
  Map<String, dynamic> _whereUniqueToJson(GameVoteWhereUniqueInput where) {
    return where.toJson()..removeWhere((key, value) => value == null);
  }

  /// Convert WhereInput to JSON for JsonQueryBuilder
  Map<String, dynamic> _whereToJson(GameVoteWhereInput where) {
      ...
      if (entry.key == 'AND' || entry.key == 'OR') {
        ...
        if (list != null && list.isNotEmpty) {
          result[entry.key] = list.map((item) {
            ...
            return (item as GameVoteWhereInput).toJson();
          }).toList();
        }
        ...
        } else if (not is GameVoteWhereInput) {
  ...
  Map<String, dynamic> _orderByToJson(GameVoteOrderByInput orderBy) {
